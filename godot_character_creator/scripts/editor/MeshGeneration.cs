...
namespace GCC
{
	...
	public partial class MeshGeneration : Node
	{
		...
		public static string CheckOriginalMeshPath(string cloth)
		{
			...
			{
				var meshPath = CharacterData.resourceBasePath+"/meshes_for_generation/" + cloth + format;
		...
		public async override void _Ready()
		{
			...
			var skeletonScene = (PackedScene)ResourceLoader.Load(CharacterData.resourceBasePath+"/assets/skeletons/default.tscn");
			var skeleton = skeletonScene.Instantiate() as Skeleton3D;
			var excludedBones = new string[] { "lolid.L", "lolid.R", "uplid.L", "uplid.R" }; //Removed  "LeftEye", "RightEye", "eye.L.001", "eye.R.001", "Jaw" because that caused bleeding
		...
		{
			DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/assets/materials/");
			...
			DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/assets/meshes/body/");
			ResourceSaver.Save(mesh, CharacterData.resourceBasePath+"/assets/meshes/body/body.mesh");
			MakeScene("body/body");
			...
			var vertexGroupsTexture = ResourceLoader.Load(CharacterData.resourceBasePath+"/meshes_for_generation/vertex_groups.png") as Texture2D;
		...
		private async Task ClothesWithoutGeneration(Dictionary<string, Array<Variant>> meshShapes)
		{
			...
			foreach (var cloth in clothesNormal)
			{
				...
				if (cloth.RFind("/") != -1) DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/assets/meshes/" + cloth[..cloth.RFind("/")]);
				ResourceSaver.Save(mesh, CharacterData.resourceBasePath+"/assets/meshes/" + cloth + ".mesh");
				MakeScene(cloth);
		...
		private async Task ClothesWithFullGeneration(Array<Vector3[]> processedBodyShapes, Vector3[] bodyBaseForm, Dictionary<string, Array<Variant>> meshShapes, Dictionary<string, int> bodyShapeNameIndex, int[] bodyBones, float[] bodyBoneWeights, int[] excludedBoneIdsEyelashes, int[] excludedBoneIds, int excludedBonesReplacement)
		{
			...
			for (var i = 0; i < length; i++)
			{
				var cloth = clothesGeneration[i];
				if (cloth.RFind("/") != -1) DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/assets/meshes/" + cloth[..cloth.RFind("/")]);
				ResourceSaver.Save(meshArray[i], CharacterData.resourceBasePath+"/assets/meshes/" + cloth + ".mesh");
				MakeScene(cloth);
			}
		}
		...
		{
			DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/blend_shape_data/");

			foreach (var dataPairs in data)
			{
				var clothName = dataPairs.Key;
				var file = FileAccess.OpenCompressed(CharacterData.resourceBasePath+"/blend_shape_data/" + clothName + ".txt", FileAccess.ModeFlags.Write, FileAccess.CompressionMode.Zstd);
		...
		{
			if (cloth.RFind("/") != -1) DirAccess.MakeDirRecursiveAbsolute(CharacterData.resourceBasePath+"/assets/materials/" + cloth[..cloth.RFind("/")]);

			skin ??= ResourceLoader.Load<Skin>(CharacterData.resourceBasePath+"/assets/skins/body.tres");
